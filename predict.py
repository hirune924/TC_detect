...
def main():
    ...
    dataset = tf.data.TFRecordDataset(["03_TFrecord/test_1.tfrecord","03_TFrecord/test_2.tfrecord","03_TFrecord/test_3.tfrecord"])\
        .map(parse_function)\
        .map(read_image)\
        .batch(1)
    ...
    # 初期化を行うための計算グラフを作成する。
    res = model(next_elem[0], next_elem[1])
    saver = tf.train.Saver()
    step = 0
    submit = ""
    with tf.Session() as sess:
        saver.restore(sess,"./ckpt/rev01/model.ckpt")
        while True:
            try:
